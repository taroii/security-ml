...
Per responses.md Section 7, with the user's revised plan:
  ...
  2. Empirical H+ win rate and half-integer score sweep across k at fixed
     sigma=0.05. Loaded from results/attack_k{0,1,5}_sigma0.0500.csv
     (produced by membership_inference.py).

The juxtaposition is the point: signal mass is roughly constant in k, yet
H+ win rate still drops with k -- because spreading the signal across
more samples compounds the masking from Pi_1 and +B.
"""
...
if __name__ == "__main__":
    ...
    for k in K_AVAILABLE:
        ...
        abl_rows.append({
            "k": k,
            "h_plus_win_rate": float(df["h_plus_win_rate"].iloc[0]),
            "h_plus_win_se": float(df["h_plus_win_se"].iloc[0]),
            "score_half": float(df["score_half"].iloc[0]),
            "score_half_se": float(df["score_half_std"].iloc[0]) / np.sqrt(n_tgt),
        })
    ...
    ax = axes[0]
    ax.errorbar(
        abl["k"], abl["h_plus_win_rate"], yerr=abl["h_plus_win_se"],
    ...
    ax.set_ylabel("$H^+$ win rate")
    ax.set_title("(a) Attack discrimination")
    ...
    ax = axes[1]
    ax.errorbar(
        abl["k"], abl["score_half"], yerr=abl["score_half_se"],
        marker="s", capsize=4, color="darkorange", linewidth=1.5,
    )
    ax.set_xlabel("$k$ (mixing parameter)")
    ax.set_ylabel("Adversary score (half-integer)")
    ax.set_title("(b) Frame-recovery score")
    ax.set_xticks(abl["k"])
    upper = max(abl["score_half"].max() * 1.3, 0.2)
